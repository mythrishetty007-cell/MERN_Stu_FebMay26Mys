...
function checkDuplicateBooking(movie,showtime,seatCount){
    return new Promise((resolve,reject)=>{
        setTimeout(() => {
            ...
            resolve("No Duplicate booking focused.");
        },300);
    });
}
...
function generateBookingDeatils(movie,showtime,seatCount){
...
async function processBookingAsync(movie,showtime,seatCount){
    try{
        ...
        const booking = await generateBookingDeatils(movie,showtime,seatCount);
        ...
        bookingEmitter.emit("bookigFailed",error);
        throw error;
    }
}
